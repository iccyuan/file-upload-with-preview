...
class U {
  constructor(A, g = {}) {
    ...
    const { browse: t, chooseFile: n, label: l, selectedCount: c } = g.text || {};
    this.options.text.chooseFile = n ?? this.options.text.chooseFile, this.options.text.browse = t ?? this.options.text.browse, this.options.text.label = l ?? r, this.options.text.selectedCount = c ?? this.options.text.selectedCount;
  ...
  `;
    const s = this.el.querySelector(".custom-file-container .input-hidden"), o = this.el.querySelector(".custom-file-container .image-preview");
    if (s != null && o != null)
      this.inputHidden = s, this.imagePreview = o;
    else
      throw new Error(`Cannot find all necessary elements for the id: ${this.uploadId}`);
    const { backgroundImage: m, baseImage: d, successFileAltImage: S, successPdfImage: u, successVideoImage: h } = g.images || {};
    this.options.images.baseImage = d ?? this.options.images.baseImage, this.options.images.successPdfImage = u ?? this.options.images.successPdfImage, this.options.images.successVideoImage = h ?? this.options.images.successVideoImage, this.options.images.successFileAltImage = S ?? this.options.images.successFileAltImage, this.options.images.backgroundImage = m ?? this.options.images.backgroundImage, this.addImagesFromPath(this.options.presetFiles), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.bindClickEvents();
  }
  bindClickEvents() {
    this.inputHidden.addEventListener(
      ...
      if (g.matches(".custom-file-container .image-preview-item-clear-icon")) {
        const B = g.getAttribute("data-upload-name"), C = this.cachedFileArray.findIndex(({ name: E }) => E === B);
        this.deleteFileAtIndex(C), A.stopPropagation();
      } else
        g.matches(".image-preview") && this.emulateInputSelection();
    });
  }
  ...
  addFileToPreviewPanel(A) {
    ...
    >
      ...
      <div class="image-preview-item-progress-bar-container">
        <div class="image-preview-item-progress-bar" data-upload-name="${A.name}-progress"></div>
      </div>
    </div>
      `;
    };
  ...
  deleteFileAtIndex(A) {
    ...
    const [g] = this.cachedFileArray.splice(A, 1), B = this.imagePreview.querySelector(
      `.image-preview-item[data-upload-name="${g.name}"]`
    );
    B && this.imagePreview.removeChild(B), this.cachedFileArray.length === 0 && (this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`);
    const C = {
      detail: {
        cachedFileArray: this.cachedFileArray,
    ...
    }, E = new CustomEvent(e.IMAGE_DELETED, C);
    window.dispatchEvent(E);
  }
  refreshPreviewPanel() {
    const g = this.imagePreview.querySelectorAll(".image-preview-item");
    Array.from(g).forEach((C) => C.classList.add(f)), setTimeout(() => {
  ...
  setPorgress(A, g) {
    const B = this.imagePreview.querySelector(
      `.image-preview-item-progress-bar[data-upload-name="${A.name}-progress"]`
    );
    B && g < 100 && (B.style.width = `${g}%`);
  }
  emulateInputSelection() {
    this.inputHidden.click();
  }
  resetPreviewPanel() {
    this.inputHidden.value = "", this.imagePreview.innerHTML = "", this.cachedFileArray = [];
  }
}
export {
  v as DEFAULT_BACKGROUND_IMAGE,
  J as DEFAULT_BASE_IMAGE,
  p as DEFAULT_SUCCESS_FILE_ALT_IMAGE,
  R as DEFAULT_SUCCESS_PDF_IMAGE,
  w as DEFAULT_SUCCESS_VIDEO_IMAGE,
  e as Events,
  U as FileUploadWithPreview
};
