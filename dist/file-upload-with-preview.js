...
class S {
  constructor(A, e = {}) {
    ...
    const { accept: g, maxFileCount: Q, multiple: I, presetFiles: B, showDeleteButtonOnImages: E } = e;
    this.options.showDeleteButtonOnImages = E ?? !0, this.options.maxFileCount = Q ?? 0, this.options.presetFiles = B ?? [], this.options.multiple = I ?? !1, this.options.accept = g ?? this.options.accept;
    ...
    const { backgroundImage: s, baseImage: r, successFileAltImage: n, successPdfImage: l, successVideoImage: c } = e.images || {};
    this.options.images.baseImage = r ?? this.options.images.baseImage, this.options.images.successPdfImage = l ?? this.options.images.successPdfImage, this.options.images.successVideoImage = c ?? this.options.images.successVideoImage, this.options.images.successFileAltImage = n ?? this.options.images.successFileAltImage, this.options.images.backgroundImage = s ?? this.options.images.backgroundImage, this.addImagesFromPath(this.options.presetFiles), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.bindClickEvents();
  }
  bindClickEvents() {
    this.inputHidden.addEventListener(
      "change",
      (A) => {
        const e = A.target, { files: g } = e;
        g != null && (this.addFiles(g), e.value = "");
      },
      !0
      ...
      if (e) {
        if (e.matches(".custom-file-container .image-preview-item-clear-icon")) {
          const g = e.getAttribute("data-upload-name"), Q = this.cachedFileArray.findIndex(({ name: I }) => I === g);
          this.deleteFileAtIndex(Q);
        }
        if (e.matches(".custom-file-container .image-preview-item")) {
          const g = e.querySelector(".image-preview-item-clear-icon"), Q = g == null ? void 0 : g.getAttribute("data-upload-name"), I = this.cachedFileArray.findIndex(({ name: i }) => i === Q);
          if (I < 0)
            return;
          const B = {
            detail: {
              cachedFileArray: this.cachedFileArray,
              file: this.cachedFileArray[I],
              index: I,
              uploadId: this.uploadId
            }
          }, E = new CustomEvent(t.IMAGE_MULTI_ITEM_CLICKED, B);
  ...
  async addImagesFromPath(A) {
    A.forEach(async (e) => {
      try {
        const g = "image/jpeg", I = await (await fetch(e, { mode: "cors" })).blob(), B = new File([I], "preset-file", {
          type: I.type || g
        });
        this.addFiles([B]);
      } catch (g) {
        g instanceof Error && console.warn(`${g.message.toString()}`), console.warn("Image cannot be added to the cachedFileArray.");
      }
    });
  }
  addFiles(A) {
    if (!A.length)
      return;
    let e = Array.from(A), g = !1;
    if (e.forEach((B) => {
      this.cachedFileArray.some((E) => E.name === B.name) && (g = !0);
    }), g)
      return;
    if (this.options.multiple && this.options.maxFileCount > 0) {
      const E = this.cachedFileArray.length + e.length - this.options.maxFileCount;
      E > 0 && (e = e.slice(0, e.length - E));
    }
    this.options.multiple || (this.cachedFileArray = []), e.forEach((B) => {
      const E = new File([B], `${B.name}`, {
        type: B.type
      });
      this.cachedFileArray.push(E), this.addFileToPreviewPanel(E);
    });
    const Q = {
      detail: {
    ...
    }, I = new CustomEvent(t.IMAGE_ADDED, Q);
  ...
  addFileToPreviewPanel(A) {
    ...
    e.readAsDataURL(A), e.onload = () => {
      ...
      A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? Q = e.result : A.type.match("application/pdf") ? Q = this.options.images.successPdfImage : A.type.match("video/*") && (Q = this.options.images.successVideoImage), this.imagePreview.innerHTML += `
        ...
        >
          ${this.options.showDeleteButtonOnImages ? g(A.name) : void 0}
        </div>
      `;
    };
  }
  ...
  deleteFileAtIndex(A) {
    ...
    const [e] = this.cachedFileArray.splice(A, 1), g = this.imagePreview.querySelector(
      `.image-preview-item[data-upload-name="${e.name}"]`
    );
    g && this.imagePreview.removeChild(g), this.cachedFileArray.length === 0 && (this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`);
    const Q = {
      detail: {
        cachedFileArray: this.cachedFileArray,
    ...
    }, I = new CustomEvent(t.IMAGE_DELETED, Q);
    window.dispatchEvent(I);
  }
  refreshPreviewPanel() {
    const e = this.imagePreview.querySelectorAll(".image-preview-item");
    Array.from(e).forEach((Q) => Q.classList.add(p)), setTimeout(() => {
