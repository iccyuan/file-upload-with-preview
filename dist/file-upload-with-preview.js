var i = /* @__PURE__ */ ((C) => (C.IMAGE_ADDED = "fileUploadWithPreview:imagesAdded", C.IMAGE_DELETED = "fileUploadWithPreview:imageDeleted", C.CLEAR_BUTTON_CLICKED = "fileUploadWithPreview:clearButtonClicked", C.IMAGE_MULTI_ITEM_CLICKED = "fileUploadWithPreview:imageMultiItemClicked", C))(i || {});
const u = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wIAAAgBAQABXenwAAAAAElFTkSuQmCC", p = "data:image/png;base64,", v = "data:image/png;base64,", f = "data:image/png;base64,", S = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wIAAAgBAQABXenwAAAAAElFTkSuQmCC", w = "multi-item-clear-animation";
class R {
  constructor(A, e = {}) {
    if (this.options = {
      accept: "*",
      images: {
        backgroundImage: S,
        baseImage: u,
        successFileAltImage: f,
        successPdfImage: p,
        successVideoImage: v
      },
      maxFileCount: 0,
      multiple: !1,
      presetFiles: [],
      showDeleteButtonOnImages: !0,
      showSelectFileButton: !0
    }, !A)
      throw new Error(
        "No uploadId found. You must initialize file-upload-with-preview with a unique uploadId."
      );
    this.uploadId = A, this.cachedFileArray = [];
    const {
      accept: g,
      maxFileCount: Q,
      multiple: I,
      presetFiles: B,
      showDeleteButtonOnImages: E,
      showSelectFileButton: t
    } = e;
    this.options.showDeleteButtonOnImages = E ?? !0, this.options.maxFileCount = Q ?? 0, this.options.presetFiles = B ?? [], this.options.multiple = I ?? !1, this.options.accept = g ?? this.options.accept, this.options.showSelectFileButton = t ?? !0;
    const a = document.querySelector(`.custom-file-container[data-upload-id="${this.uploadId}"]`);
    if (!a)
      throw new Error(`Could not find a 'custom-file-container' with the id of: ${this.uploadId}`);
    this.el = a, this.el.innerHTML += `
      <label class="input-container">
        <input
          accept="${this.options.accept}"
          aria-label="Choose File"
          class="input-hidden"
          id="file-upload-with-preview-${A}"
          ${this.options.multiple ? "multiple" : ""}
          type="file"
        />
      </label>
      <div class="image-preview"></div>
    `;
    const o = this.el.querySelector(".input-container"), s = this.el.querySelector(".custom-file-container .input-hidden"), n = this.el.querySelector(".custom-file-container .image-preview"), r = s != null && n != null;
    if (e.showSelectFileButton ? o.style.display = "block" : o.style.display = "none", r)
      this.inputHidden = s, this.imagePreview = n;
    else
      throw new Error(`Cannot find all necessary elements for the id: ${this.uploadId}`);
    const { backgroundImage: l, baseImage: c, successFileAltImage: d, successPdfImage: m, successVideoImage: h } = e.images || {};
    this.options.images.baseImage = c ?? this.options.images.baseImage, this.options.images.successPdfImage = m ?? this.options.images.successPdfImage, this.options.images.successVideoImage = h ?? this.options.images.successVideoImage, this.options.images.successFileAltImage = d ?? this.options.images.successFileAltImage, this.options.images.backgroundImage = l ?? this.options.images.backgroundImage, this.addImagesFromPath(this.options.presetFiles), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.bindClickEvents();
  }
  bindClickEvents() {
    this.inputHidden.addEventListener(
      "change",
      (A) => {
        const e = A.target, { files: g } = e;
        g != null && (this.addFiles(g), e.value = "");
      },
      !0
    ), this.imagePreview.addEventListener("click", (A) => {
      const e = A.target;
      if (e) {
        if (e.matches(".custom-file-container .image-preview-item-clear-icon")) {
          const g = e.getAttribute("data-upload-name"), Q = this.cachedFileArray.findIndex(({ name: I }) => I === g);
          this.deleteFileAtIndex(Q);
        }
        if (e.matches(".custom-file-container .image-preview-item")) {
          const g = e.querySelector(".image-preview-item-clear-icon"), Q = g == null ? void 0 : g.getAttribute("data-upload-name"), I = this.cachedFileArray.findIndex(({ name: t }) => t === Q);
          if (I < 0)
            return;
          const B = {
            detail: {
              cachedFileArray: this.cachedFileArray,
              file: this.cachedFileArray[I],
              index: I,
              uploadId: this.uploadId
            }
          }, E = new CustomEvent(i.IMAGE_MULTI_ITEM_CLICKED, B);
          window.dispatchEvent(E);
        }
      }
    });
  }
  async addImagesFromPath(A) {
    A.forEach(async (e) => {
      try {
        const g = "image/jpeg", I = await (await fetch(e, { mode: "cors" })).blob(), B = new File([I], "preset-file", {
          type: I.type || g
        });
        this.addFiles([B]);
      } catch (g) {
        g instanceof Error && console.warn(`${g.message.toString()}`), console.warn("Image cannot be added to the cachedFileArray.");
      }
    });
  }
  addFiles(A) {
    if (!A.length)
      return;
    let e = Array.from(A), g = !1;
    if (e.forEach((B) => {
      this.cachedFileArray.some((E) => E.name === B.name) && (g = !0);
    }), g)
      return;
    if (this.options.multiple && this.options.maxFileCount > 0) {
      const E = this.cachedFileArray.length + e.length - this.options.maxFileCount;
      E > 0 && (e = e.slice(0, e.length - E));
    }
    this.options.multiple || (this.cachedFileArray = []), e.forEach((B) => {
      const E = new File([B], `${B.name}`, {
        type: B.type
      });
      this.cachedFileArray.push(E), this.addFileToPreviewPanel(E);
    });
    const Q = {
      detail: {
        addedFilesCount: e.length,
        cachedFileArray: this.cachedFileArray,
        files: A,
        uploadId: this.uploadId
      }
    }, I = new CustomEvent(i.IMAGE_ADDED, Q);
    window.dispatchEvent(I);
  }
  addFileToPreviewPanel(A) {
    const e = new FileReader();
    e.readAsDataURL(A), e.onload = () => {
      if (!this.options.multiple) {
        let I = this.options.images.successFileAltImage;
        A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? I = `url("${e.result}")` : A.type.match("application/pdf") ? I = `url("${this.options.images.successPdfImage}")` : A.type.match("video/*") && (I = `url("${this.options.images.successVideoImage}")`), this.imagePreview.style.backgroundImage = I;
        return;
      }
      this.imagePreview.style.backgroundImage = `url("${this.options.images.backgroundImage}")`;
      const g = (I) => `
        <span class="image-preview-item-clear">
          <span class="image-preview-item-clear-icon" data-upload-name="${I}">
            &times;
          </span>
        </span>
      `;
      let Q = this.options.images.successFileAltImage;
      A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? Q = e.result : A.type.match("application/pdf") ? Q = this.options.images.successPdfImage : A.type.match("video/*") && (Q = this.options.images.successVideoImage), this.imagePreview.innerHTML += `
        <div
          class="image-preview-item"
          data-upload-name="${A.name}"
          style="background-image: url('${Q}'); "
        >
          ${this.options.showDeleteButtonOnImages ? g(A.name) : void 0}
        </div>
      `;
    };
  }
  replaceFiles(A) {
    if (!A.length)
      throw new Error("Array must contain at least one file.");
    this.cachedFileArray = A, this.refreshPreviewPanel();
  }
  replaceFileAtIndex(A, e) {
    if (!this.cachedFileArray[e])
      throw new Error(`There is no file at index: ${e}`);
    this.cachedFileArray[e] = A, this.refreshPreviewPanel();
  }
  deleteFileAtIndex(A) {
    if (!this.cachedFileArray[A])
      throw new Error(`There is no file at index ${A}`);
    const [e] = this.cachedFileArray.splice(A, 1), g = this.imagePreview.querySelector(
      `.image-preview-item[data-upload-name="${e.name}"]`
    );
    g && this.imagePreview.removeChild(g), this.cachedFileArray.length === 0 && (this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`);
    const Q = {
      detail: {
        cachedFileArray: this.cachedFileArray,
        currentFileCount: this.cachedFileArray.length,
        index: A,
        uploadId: this.uploadId
      }
    }, I = new CustomEvent(i.IMAGE_DELETED, Q);
    window.dispatchEvent(I);
  }
  refreshPreviewPanel() {
    const e = this.imagePreview.querySelectorAll(".image-preview-item");
    Array.from(e).forEach((Q) => Q.classList.add(w)), setTimeout(() => {
      if (this.imagePreview.innerHTML = "", !this.cachedFileArray.length) {
        this.resetPreviewPanel();
        return;
      }
      this.cachedFileArray.forEach((Q) => this.addFileToPreviewPanel(Q));
    }, 200);
  }
  emulateInputSelection() {
    this.inputHidden.click();
  }
  resetPreviewPanel() {
    this.inputHidden.value = "", this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.imagePreview.innerHTML = "", this.cachedFileArray = [];
  }
}
export {
  S as DEFAULT_BACKGROUND_IMAGE,
  u as DEFAULT_BASE_IMAGE,
  f as DEFAULT_SUCCESS_FILE_ALT_IMAGE,
  p as DEFAULT_SUCCESS_PDF_IMAGE,
  v as DEFAULT_SUCCESS_VIDEO_IMAGE,
  i as Events,
  R as FileUploadWithPreview
};
