var i = /* @__PURE__ */ ((E) => (E.IMAGE_ADDED = "fileUploadWithPreview:imagesAdded", E.IMAGE_DELETED = "fileUploadWithPreview:imageDeleted", E.CLEAR_BUTTON_CLICKED = "fileUploadWithPreview:clearButtonClicked", E.IMAGE_MULTI_ITEM_CLICKED = "fileUploadWithPreview:imageMultiItemClicked", E))(i || {});
const d = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wIAAAgBAQABXenwAAAAAElFTkSuQmCC", m = "data:image/png;base64,", h = "data:image/png;base64,", u = "data:image/png;base64,", v = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wIAAAgBAQABXenwAAAAAElFTkSuQmCC", p = ":upload:", f = "multi-item-clear-animation", S = () => Math.random().toString(16).slice(2);
class R {
  constructor(A, e = {}) {
    if (this.options = {
      accept: "*",
      images: {
        backgroundImage: v,
        baseImage: d,
        successFileAltImage: u,
        successPdfImage: m,
        successVideoImage: h
      },
      maxFileCount: 0,
      multiple: !1,
      presetFiles: [],
      showDeleteButtonOnImages: !0
    }, !A)
      throw new Error(
        "No uploadId found. You must initialize file-upload-with-preview with a unique uploadId."
      );
    this.uploadId = A, this.cachedFileArray = [];
    const { accept: I, maxFileCount: Q, multiple: g, presetFiles: B, showDeleteButtonOnImages: t } = e;
    this.options.showDeleteButtonOnImages = t ?? !0, this.options.maxFileCount = Q ?? 0, this.options.presetFiles = B ?? [], this.options.multiple = g ?? !1, this.options.accept = I ?? this.options.accept;
    const C = document.querySelector(`.custom-file-container[data-upload-id="${this.uploadId}"]`);
    if (!C)
      throw new Error(`Could not find a 'custom-file-container' with the id of: ${this.uploadId}`);
    this.el = C, this.el.innerHTML += `
      <label class="input-container">
        <input
          accept="${this.options.accept}"
          aria-label="Choose File"
          class="input-hidden"
          id="file-upload-with-preview-${A}"
          ${this.options.multiple ? "multiple" : ""}
          type="file"
        />
      </label>
      <div class="image-preview"></div>
    `;
    const a = this.el.querySelector(".custom-file-container .input-hidden"), o = this.el.querySelector(".custom-file-container .image-preview");
    if (a != null && o != null)
      this.inputHidden = a, this.imagePreview = o;
    else
      throw new Error(`Cannot find all necessary elements for the id: ${this.uploadId}`);
    const { backgroundImage: s, baseImage: n, successFileAltImage: r, successPdfImage: l, successVideoImage: c } = e.images || {};
    this.options.images.baseImage = n ?? this.options.images.baseImage, this.options.images.successPdfImage = l ?? this.options.images.successPdfImage, this.options.images.successVideoImage = c ?? this.options.images.successVideoImage, this.options.images.successFileAltImage = r ?? this.options.images.successFileAltImage, this.options.images.backgroundImage = s ?? this.options.images.backgroundImage, this.addImagesFromPath(this.options.presetFiles), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.bindClickEvents();
  }
  bindClickEvents() {
    this.inputHidden.addEventListener(
      "change",
      (A) => {
        const e = A.target, { files: I } = e;
        I != null && (this.addFiles(I), e.value = "");
      },
      !0
    ), this.imagePreview.addEventListener("click", (A) => {
      const e = A.target;
      if (e) {
        if (e.matches(".custom-file-container .image-preview-item-clear-icon")) {
          const I = e.getAttribute("data-upload-name"), Q = this.cachedFileArray.findIndex(({ name: g }) => g === I);
          this.deleteFileAtIndex(Q);
        }
        if (e.matches(".custom-file-container .image-preview-item")) {
          const I = e.querySelector(".image-preview-item-clear-icon"), Q = I == null ? void 0 : I.getAttribute("data-upload-name"), g = this.cachedFileArray.findIndex(({ name: C }) => C === Q);
          if (g < 0)
            return;
          const B = {
            detail: {
              cachedFileArray: this.cachedFileArray,
              file: this.cachedFileArray[g],
              index: g,
              uploadId: this.uploadId
            }
          }, t = new CustomEvent(i.IMAGE_MULTI_ITEM_CLICKED, B);
          window.dispatchEvent(t);
        }
      }
    });
  }
  async addImagesFromPath(A) {
    A.forEach(async (e) => {
      try {
        const I = "image/jpeg", g = await (await fetch(e, { mode: "cors" })).blob(), B = new File([g], "preset-file", {
          type: g.type || I
        });
        this.addFiles([B]);
      } catch (I) {
        I instanceof Error && console.warn(`${I.message.toString()}`), console.warn("Image cannot be added to the cachedFileArray.");
      }
    });
  }
  addFiles(A) {
    if (!A.length)
      return;
    let e = Array.from(A);
    if (this.options.multiple && this.options.maxFileCount > 0) {
      const B = this.cachedFileArray.length + e.length - this.options.maxFileCount;
      B > 0 && (e = e.slice(0, e.length - B));
    }
    this.options.multiple || (this.cachedFileArray = []), e.forEach((g) => {
      const B = new File(
        [g],
        `${g.name || "fallback-name"}${p}${S()}`,
        {
          type: g.type
        }
      );
      this.cachedFileArray.push(B), this.addFileToPreviewPanel(B);
    });
    const I = {
      detail: {
        addedFilesCount: e.length,
        cachedFileArray: this.cachedFileArray,
        files: A,
        uploadId: this.uploadId
      }
    }, Q = new CustomEvent(i.IMAGE_ADDED, I);
    window.dispatchEvent(Q);
  }
  addFileToPreviewPanel(A) {
    const e = new FileReader();
    e.readAsDataURL(A), e.onload = () => {
      if (!this.options.multiple) {
        let g = this.options.images.successFileAltImage;
        A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? g = `url("${e.result}")` : A.type.match("application/pdf") ? g = `url("${this.options.images.successPdfImage}")` : A.type.match("video/*") && (g = `url("${this.options.images.successVideoImage}")`), this.imagePreview.style.backgroundImage = g;
        return;
      }
      this.imagePreview.style.backgroundImage = `url("${this.options.images.backgroundImage}")`;
      const I = (g) => `
        <span class="image-preview-item-clear">
          <span class="image-preview-item-clear-icon" data-upload-name="${g}">
            &times;
          </span>
        </span>
      `;
      let Q = this.options.images.successFileAltImage;
      A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? Q = e.result : A.type.match("application/pdf") ? Q = this.options.images.successPdfImage : A.type.match("video/*") && (Q = this.options.images.successVideoImage), this.imagePreview.innerHTML += `
        <div
          class="image-preview-item"
          data-upload-name="${A.name}"
          style="background-image: url('${Q}'); "
        >
          ${this.options.showDeleteButtonOnImages ? I(A.name) : void 0}
        </div>
      `;
    };
  }
  replaceFiles(A) {
    if (!A.length)
      throw new Error("Array must contain at least one file.");
    this.cachedFileArray = A, this.refreshPreviewPanel();
  }
  replaceFileAtIndex(A, e) {
    if (!this.cachedFileArray[e])
      throw new Error(`There is no file at index: ${e}`);
    this.cachedFileArray[e] = A, this.refreshPreviewPanel();
  }
  deleteFileAtIndex(A) {
    if (!this.cachedFileArray[A])
      throw new Error(`There is no file at index ${A}`);
    const [e] = this.cachedFileArray.splice(A, 1), I = this.imagePreview.querySelector(
      `.image-preview-item[data-upload-name="${e.name}"]`
    );
    I && this.imagePreview.removeChild(I), this.cachedFileArray.length === 0 && (this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`);
    const Q = {
      detail: {
        cachedFileArray: this.cachedFileArray,
        currentFileCount: this.cachedFileArray.length,
        index: A,
        uploadId: this.uploadId
      }
    }, g = new CustomEvent(i.IMAGE_DELETED, Q);
    window.dispatchEvent(g);
  }
  refreshPreviewPanel() {
    const e = this.imagePreview.querySelectorAll(".image-preview-item");
    Array.from(e).forEach((Q) => Q.classList.add(f)), setTimeout(() => {
      if (this.imagePreview.innerHTML = "", !this.cachedFileArray.length) {
        this.resetPreviewPanel();
        return;
      }
      this.cachedFileArray.forEach((Q) => this.addFileToPreviewPanel(Q));
    }, 200);
  }
  emulateInputSelection() {
    this.inputHidden.click();
  }
  resetPreviewPanel() {
    this.inputHidden.value = "", this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.imagePreview.innerHTML = "", this.cachedFileArray = [];
  }
}
export {
  v as DEFAULT_BACKGROUND_IMAGE,
  d as DEFAULT_BASE_IMAGE,
  u as DEFAULT_SUCCESS_FILE_ALT_IMAGE,
  m as DEFAULT_SUCCESS_PDF_IMAGE,
  h as DEFAULT_SUCCESS_VIDEO_IMAGE,
  i as Events,
  R as FileUploadWithPreview
};
